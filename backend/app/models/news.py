...
class NewsArticle(TimestampMixin, table=True):
    """新闻资讯表 — 业务表（无前缀）"""
    ...
    cover_image: Optional[str] = Field(
        default=None, max_length=500, description="封面图"
    )
    category: Optional[str] = Field(default=None, max_length=50, description="分类")
    tags: Optional[str] = Field(
        default=None, max_length=500, description="标签(JSON数组字符串)"
    )
    status: str = Field(
        default="draft", max_length=20, description="状态: draft / published / archived"
    )
