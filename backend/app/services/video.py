...
def get_videos(
    ...
    statement = statement.order_by(
        col(OperationVideo.sort_order).asc(), col(OperationVideo.created_at).desc()
    )
...
def get_video_by_id(
    session: Session, id: int, increment_view: bool = False
) -> OperationVideo:
    """
    ...
    if not video:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"操作视频 ID #{id} 不存在"
        )
